...
function BcGrid({children}: PropsWithChildren<{}>): JSX.Element {
  ...
  return (
    <div className={styles.Grid}>
      {React.Children.map(children, child => React.isValidElement(child) ? React.cloneElement(child, {inGrid: true}) : child)}
      {missingChildren === 2 ? <><div/><div/></> : missingChildren === 1 ? <div/> : <></>}
    </div>
  )
}
